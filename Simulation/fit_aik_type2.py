...
for na12m in range(1,5):
	for na16m in range(1,5):
		...
		save_data_path = 'E:/02_AIS/Simulation/AIS/190708/fit_v2/data/aik'+str(na12m)+str(na16m)+'/'
		save_figure_path = 'E:/02_AIS/Simulation/AIS/190708/fit_v2/figure/aik'+str(na12m)+str(na16m)+'/'
		...
		for i,gk in enumerate(ais_gk_x):		
			for j, l in enumerate(len_x):
				print('na12m:',na12m, '; na16m:',na16m, ';gk:', gk, 'pS/um2; len:', l,'um;')		
				cell = nm.NeuronModel(ail = int(l), na_type=2, na12_map=na12m, na16_map=na16m, aik=gk)
				stim = nm.attach_current_clamp(cell, amp=.7)
				vec = nm.set_recording_vectors(cell)
				nm.simulate()
				nm.save_vec(vec, save_data_path+str(na12m)+str(na16m)+'_gk_'+str(gk)+'_len_'+str(l)+'_')
				nm.show_output(vec, save_figure_path+str(na12m)+str(na16m)+'gk_'+str(gk)+'_len_'+str(l)+'_')
			...
			for j, d in enumerate(dist_x):
				print('na12m:',na12m, '; na16m:',na16m, ';gk:', gk, 'pS/um2; dist:', d,'um;')		
				cell = nm.NeuronModel(hl=int(d), na_type=2, na12_map=na12m, na16_map=na16m, aik=gk)
				stim = nm.attach_current_clamp(cell, amp=.7)
				vec = nm.set_recording_vectors(cell)
				nm.simulate()
				nm.save_vec(vec, save_data_path+str(na12m)+str(na16m)+'_gk_'+str(gk)+'_dist_'+str(d)+'_')
				nm.show_output(vec, save_figure_path+str(na12m)+str(na16m)+'_gk_'+str(gk)+'_dist_'+str(d)+'_')
