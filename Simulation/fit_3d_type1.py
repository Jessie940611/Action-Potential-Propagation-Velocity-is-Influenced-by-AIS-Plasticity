...
for p in aina_x+[aina]:
	...
	for i,d in enumerate(dist_x):
		for j,l in enumerate(len_x):	
			...
			nm.simulate()
			nm.save_vec(vec, save_data_path+'aina_'+str(p)+'_aik_'+str(aik)+'_dist_'+str(d)+'_len_'+str(l)+'_')
			nm.show_output(vec, save_figure_path+'aina_'+str(p)+'_aik_'+str(aik)+'_dist_'+str(d)+'_len_'+str(l)+'_')
			...
			for sec in h.allsec():
				print(sec)	
	with open(save_data_path+'aina_'+str(p)+'_aik_'+str(aik)+'_bpv.p', 'wb') as file:
		pickle.dump(bpv_sum, file)
	with open(save_data_path+'aina_'+str(p)+'_aik_'+str(aik)+'_fwv.p', 'wb') as file:
...
for p in aik_x:
	...
	for i,d in enumerate(dist_x):
		for j,l in enumerate(len_x):	
			...
			vec = nm.set_recording_vectors(cell)
			nm.simulate()
			nm.save_vec(vec, save_data_path+'aina_'+str(aina)+'_aik_'+str(p)+'_dist_'+str(d)+'_len_'+str(l)+'_')
			nm.show_output(vec, save_figure_path+'aina_'+str(aina)+'_aik_'+str(p)+'_dist_'+str(d)+'_len_'+str(l)+'_')
			bpv_sum[i,j], fwv_sum[i,j], ___ = nm.cal_velocity(vec, ['ais'])
			cell.dend1 = None
			cell.dend2 = None
			cell.soma = None
			...
			for sec in h.allsec():
				print(sec)	
	with open(save_data_path+'aina_'+str(aina)+'_aik_'+str(p)+'_bpv.p', 'wb') as file:
		pickle.dump(bpv_sum, file)
	with open(save_data_path+'aina_'+str(aina)+'_aik_'+str(p)+'_fwv.p', 'wb') as file:
		pickle.dump(fwv_sum, file)
